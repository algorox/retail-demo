...
async function clearBitEnrich(user, context, cb) {
  ...
  clearbit.Person.find({ email: user.email, stream: true }).nodeify(
    (err, person) => {
      if (err) {
        log.error(`Error fetching data from clearbit, ${err.toString()}`, true);
        global.createAuditLog("CLEARBIT_RULE", null, err);
      }
      ...
      if (!err) {
        ...
        if (person.googleplus && person.googleplus.handle) {
          um.handles.push(
            `https://plus.google.com/${person.googleplus.handle}`
          );
        }
        ...
        global.createAuditLog("CLEARBIT_RULE", user.user_metadata);
      }
