...
<head>
    <title>Code coverage report for src/context/index.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">src/context</a> index.ts</h1>
        <div class='clearfix'>
...
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!config.AUTH0_DOMAIN) <span class="cstat-no" title="statement not covered" >missingParams.push('AUTH0_DOMAIN');</span>
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!config.AUTH0_ACCESS_TOKEN) {
<span class="cstat-no" title="statement not covered" >    if (!config.AUTH0_CLIENT_ID) <span class="cstat-no" title="statement not covered" >missingParams.push('AUTH0_CLIENT_ID');</span></span>
<span class="cstat-no" title="statement not covered" >    if (!config.AUTH0_CLIENT_SECRET) <span class="cstat-no" title="statement not covered" >missingParams.push('AUTH0_CLIENT_SECRET');</span></span>
  }
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (missingParams.length &gt; 0) {
    ...
    if (usedDeprecatedParams.length &gt; 0) {
      log.warn(
        `Usage of the ${usedDeprecatedParams.join(', ')} exclusion ${
          usedDeprecatedParams.length &gt; 1 ? <span class="branch-0 cbranch-no" title="branch not covered" >'params are' </span>: 'param is'
...
&nbsp;
  const accessToken = await (async (): Promise&lt;string&gt; =&gt; {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!!config.AUTH0_ACCESS_TOKEN) return config.AUTH0_ACCESS_TOKEN;
&nbsp;
    const authClient = <span class="cstat-no" title="statement not covered" >new AuthenticationClient({</span>
      domain: config.AUTH0_DOMAIN,
      clientId: config.AUTH0_CLIENT_ID,
      clientSecret: config.AUTH0_CLIENT_SECRET,
    });
&nbsp;
    const clientCredentials = <span class="cstat-no" title="statement not covered" >await authClient.clientCredentialsGrant({</span>
      audience: config.AUTH0_AUDIENCE
        ? config.AUTH0_AUDIENCE
        : `https://${config.AUTH0_DOMAIN}/api/v2/`,
    });
<span class="cstat-no" title="statement not covered" >    return clientCredentials.access_token;</span>
  })();
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        log.debug(`Cannot convert config.${key} to an object. Error: ${e.message}`);</span>
...
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (typeof inputFile === 'object') {
<span class="cstat-no" title="statement not covered" >    return new YAMLContext(config, mgmtClient);</span>
  }
&nbsp;
