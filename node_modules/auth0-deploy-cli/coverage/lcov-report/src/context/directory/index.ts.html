...
<head>
    <title>Code coverage report for src/context/directory/index.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../../index.html">All files</a> / <a href="index.html">src/context/directory</a> index.ts</h1>
        <div class='clearfix'>
...
&nbsp;
  async <span class="fstat-no" title="function not covered" >dump(</span>): Promise&lt;void&gt; {
    const auth0 = <span class="cstat-no" title="statement not covered" >new Auth0(this.mgmtClient, this.assets, toConfigFn(this.config));</span>
<span class="cstat-no" title="statement not covered" >    log.info('Loading Auth0 Tenant Data');</span>
<span class="cstat-no" title="statement not covered" >    await auth0.loadAll();</span>
<span class="cstat-no" title="statement not covered" >    this.assets = auth0.assets;</span>
&nbsp;
    // Clean known read only fields
<span class="cstat-no" title="statement not covered" >    this.assets = cleanAssets(this.assets, this.config);</span>
&nbsp;
...
<span class="cstat-no" title="statement not covered" >    this.assets.clientsOrig = [...(this.assets.clients || [])];</span>
&nbsp;
    // Optionally Strip identifiers
<span class="cstat-no" title="statement not covered" >    if (!this.config.AUTH0_EXPORT_IDENTIFIERS) {</span>
<span class="cstat-no" title="statement not covered" >      this.assets = stripIdentifiers(auth0, this.assets);</span>
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    await Promise.all(</span>
      Object.entries(handlers)
        .filter(<span class="fstat-no" title="function not covered" >([</span>handlerName]: [AssetTypes, DirectoryHandler&lt;any&gt;]) =&gt; {
          const excludedAssetTypes = <span class="cstat-no" title="statement not covered" >this.config.AUTH0_EXCLUDED || [];</span>
<span class="cstat-no" title="statement not covered" >          return !excludedAssetTypes.includes(handlerName);</span>
        })
        .map(<span class="fstat-no" title="function not covered" >async ([</span>name, handler]) =&gt;<span class="cstat-no" title="statement not covered" ><span class="fstat-no" title="function not covered" > {</span></span>
<span class="cstat-no" title="statement not covered" >          try {</span>
<span class="cstat-no" title="statement not covered" >            await handler.dump(this);</span>
          } catch (err) {
<span class="cstat-no" title="statement not covered" >            log.debug(err.stack);</span>
<span class="cstat-no" title="statement not covered" >            throw new Error(`Problem exporting ${name}`);</span>
          }
        })
    );
  }
