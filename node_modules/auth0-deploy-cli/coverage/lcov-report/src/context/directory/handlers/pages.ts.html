...
<head>
    <title>Code coverage report for src/context/directory/handlers/pages.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../../../index.html">All files</a> / <a href="index.html">src/context/directory/handlers</a> pages.ts</h1>
        <div class='clearfix'>
...
&nbsp;
  const pages = Object.keys(sorted).flatMap((key): Asset[] =&gt; {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!meta) {
<span class="cstat-no" title="statement not covered" >      log.warn(`Skipping pages file ${html} as missing the corresponding '.json' file`);</span>
<span class="cstat-no" title="statement not covered" >      return [];</span>
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!html) {
<span class="cstat-no" title="statement not covered" >      log.warn(`Skipping pages file ${meta} as missing corresponding '.html' file`);</span>
<span class="cstat-no" title="statement not covered" >      return [];</span>
    }
    return {
      ...loadJSON(meta, context.mappings),
...
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!pages) <span class="cstat-no" title="statement not covered" >return; </span>// Skip, nothing to dump
&nbsp;
  // Create Pages folder
  const pagesFolder = path.join(context.filePath, constants.PAGES_DIRECTORY);
...
&nbsp;
  pages.forEach((page) =&gt; {
    var metadata = { ...page };
&nbsp;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (page.name !== 'error_page' || page.html !== undefined) {
      // Dump template html to file
      const htmlFile = path.join(pagesFolder, `${page.name}.html`);
      log.info(`Writing ${htmlFile}`);
      fs.writeFileSync(htmlFile || <span class="branch-1 cbranch-no" title="branch not covered" >'',</span> page.html);
