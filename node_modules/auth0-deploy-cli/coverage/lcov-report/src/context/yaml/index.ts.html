...
<head>
    <title>Code coverage report for src/context/yaml/index.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../../index.html">All files</a> / <a href="index.html">src/context/yaml</a> index.ts</h1>
        <div class='clearfix'>
...
<a name='L162'></a><a href='#L162'>162</a></td><td class="line-coverage quiet"><span class="cline-any cline-yes">1x</span>
...
&nbsp;
    this.basePath = (() =&gt; {
      <span class="missing-if-branch" title="if path not taken" >I</span>if (!!config.AUTH0_BASE_PATH) <span class="cstat-no" title="statement not covered" >return config.AUTH0_BASE_PATH;</span>
      //@ts-ignore because this looks to be a bug, but do not want to introduce regression; more investigation needed
      return typeof configFile === 'object' ? <span class="branch-0 cbranch-no" title="branch not covered" >process.cwd() </span>: path.dirname(this.configFile);
    })();
  }
&nbsp;
  loadFile(f) {
    let toLoad = path.join(this.basePath, f);
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!isFile(toLoad)) {
...
&nbsp;
  async load() {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (typeof this.configFile === 'object') {
<span class="cstat-no" title="statement not covered" >      this.assets = this.configFile;</span>
    } else {
      try {
...
&nbsp;
  async dump(): Promise&lt;void&gt; {
    ...
    try {
      ...
      const docUrl =
<span class="cstat-no" title="statement not covered" >        'https://auth0.com/docs/deploy/deploy-cli-tool/create-and-configure-the-deploy-cli-application#modify-deploy-cli-application-scopes';</span>
      const extraMessage = <span class="cstat-no" title="statement not covered" >err.message.startsWith('Insufficient scope')</span>
        ? `\nSee ${docUrl} for more information`
        : '';
<span class="cstat-no" title="statement not covered" >      throw new Error(`Problem loading tenant data from Auth0 ${err}${extraMessage}`);</span>
    }
&nbsp;
    await Promise.all(
        ...
        .map(async ([name, handler]) =&gt; {
          try {
            ...
            <span class="missing-if-branch" title="else path not taken" >E</span>if (data) {
              <span class="missing-if-branch" title="else path not taken" >E</span>if (data[name] !== null) log.info(`Exporting ${name}`);
...
&nbsp;
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!this.config.AUTH0_EXPORT_IDENTIFIERS) {
