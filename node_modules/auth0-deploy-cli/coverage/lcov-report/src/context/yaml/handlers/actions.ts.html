...
<head>
    <title>Code coverage report for src/context/yaml/handlers/actions.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../../../index.html">All files</a> / <a href="index.html">src/context/yaml/handlers</a> actions.ts</h1>
        <div class='clearfix'>
...
&nbsp;
  return {
    actions: [
      ...actions.map((action) =&gt; ({
        ...action,
        code: parseCode(context, action.code || <span class="branch-1 cbranch-no" title="branch not covered" >'')</span>,
      })),
    ],
  };
}
...
function mapSecrets(secrets: { name: string; value: string }[]): Secret[] {
  <span class="missing-if-branch" title="if path not taken" >I</span>if (secrets &amp;&amp; secrets.length &gt; 0) {
<span class="cstat-no" title="statement not covered" >    return secrets.map(<span class="fstat-no" title="function not covered" >(s</span>ecret) =&gt; (<span class="cstat-no" title="statement not covered" >{ name: secret.name, value: secret.value })</span>);</span>
  }
  return [];
}
&nbsp;
...
async function dump(context: YAMLContext): Promise&lt;ParsedActions&gt; {
  const { actions } = context.assets;
&nbsp;
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!actions) <span class="cstat-no" title="statement not covered" >return { actions: null };</span>
&nbsp;
...
&nbsp;
  return {
    actions: filteredActions.map((action) =&gt; ({
      name: action.name,
      deployed: !!action.deployed || <span class="branch-1 cbranch-no" title="branch not covered" >!!action.all_changes_deployed,</span>
      //@ts-ignore because Action resource needs to be typed more accurately
      code: mapActionCode(context.basePath, action),
      runtime: action.runtime,
      dependencies: action.dependencies || <span class="branch-1 cbranch-no" title="branch not covered" >[],</span>
      status: action.status,
      secrets: mapSecrets(action.secrets || <span class="branch-1 cbranch-no" title="branch not covered" >[])</span>,
