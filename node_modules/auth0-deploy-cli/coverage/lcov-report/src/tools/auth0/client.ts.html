...
<head>
    <title>Code coverage report for src/tools/auth0/client.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../../index.html">All files</a> / <a href="index.html">src/tools/auth0</a> client.ts</h1>
        <div class='clearfix'>
...
<span class="cline-any cline-yes">126x</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span></td><td class="text"><pre class="prettyprint lang-js">import { PromisePoolExecutor } from 'promise-pool-executor';
...
function getEntity(rsp: ApiResponse): Asset[] {
  ...
  <span class="missing-if-branch" title="else path not taken" >E</span>if (Array.isArray(found) &amp;&amp; found.length === 1) {
    return found[0] as Asset[];
  }
<span class="cstat-no" title="statement not covered" >  throw new Error('There was an error trying to find the entity within paginate');</span>
}
&nbsp;
function <span class="fstat-no" title="function not covered" >checkpointPaginator(</span>
  client: Auth0APIClient,
  target,
  name: 'getAll'
): (arg0: CheckpointPaginationParams) =&gt; Promise&lt;Asset[]&gt; {
<span class="cstat-no" title="statement not covered" >  return <span class="fstat-no" title="function not covered" >async function (.</span>..args: [CheckpointPaginationParams]) {</span>
    const data: Asset[] = <span class="cstat-no" title="statement not covered" >[];</span>
&nbsp;
    // remove the _checkpoint_ flag
    const { checkpoint<span class="cstat-no" title="statement not covered" >, ...n</span>ewArgs<span class="cstat-no" title="statement not covered" > } = <span class="cstat-no" title="statement not covered" >_</span>.cloneDeep(args[0]);</span>
&nbsp;
    // fetch the total to validate records match
    const { total } = <span class="cstat-no" title="statement not covered" >await client.pool</span>
      .addSingleTask({
        data: newArgs,
        generator: <span class="fstat-no" title="function not covered" >(r</span>equestArgs) =&gt; <span class="cstat-no" title="statement not covered" >target[name](requestArgs),</span>
      })
      .promise();
&nbsp;
...
<span class="cstat-no" title="statement not covered" >    newArgs.take = 50;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    while (!done) {</span>
      const rsp = <span class="cstat-no" title="statement not covered" >await client.pool</span>
        .addSingleTask({
          data: newArgs,
          generator: <span class="fstat-no" title="function not covered" >(r</span>equestArgs) =&gt; <span class="cstat-no" title="statement not covered" >target[name](requestArgs),</span>
        })
        .promise();
&nbsp;
<span class="cstat-no" title="statement not covered" >      data.push(...getEntity(rsp));</span>
<span class="cstat-no" title="statement not covered" >      if (!rsp.next) {</span>
<span class="cstat-no" title="statement not covered" >        done = true;</span>
      } else {
<span class="cstat-no" title="statement not covered" >        newArgs.from = rsp.next;</span>
      }
    }
&nbsp;
<span class="cstat-no" title="statement not covered" >    if (data.length !== total) {</span>
<span class="cstat-no" title="statement not covered" >      throw new Error('Fail to load data from tenant');</span>
    }
&nbsp;
...
&nbsp;
      <span class="missing-if-branch" title="if path not taken" >I</span>if (data.length !== total) {
<span class="cstat-no" title="statement not covered" >        throw new Error('Fail to load data from tenant');</span>
      }
    }
    return data;
    ...
    get: function (target: Auth0APIClient, name: string, receiver: unknown) {
      if (name === 'getAll') {
        return async function (...args: [CheckpointPaginationParams | PagePaginationParams]) {
          switch (true) {
<span class="branch-0 cbranch-no" title="branch not covered" >            case args[0] &amp;&amp; typeof args[0] === 'object' &amp;&amp; args[0].checkpoint:</span>
<span class="cstat-no" title="statement not covered" >              return checkpointPaginator(</span>
                client,
                target,
                name
