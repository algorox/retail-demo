...
<head>
    <title>Code coverage report for src/tools/auth0/handlers/pages.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../../../index.html">All files</a> / <a href="index.html">src/tools/auth0/handlers</a> pages.ts</h1>
        <div class='clearfix'>
...
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!globalClient[0]) {
<span class="cstat-no" title="statement not covered" >      throw new Error('Unable to find global client id when trying to update the login page');</span>
    }
&nbsp;
...
&nbsp;
  async updatePages(pages: Asset[]): Promise&lt;void&gt; {
    ...
    const update = toUpdate.reduce((accum, page) =&gt; {
      <span class="missing-if-branch" title="else path not taken" >E</span>if (supportedPages.includes(page.name)) {
        const pageName = pageNameMap[page.name];
        <span class="missing-if-branch" title="if path not taken" >I</span>if (!pageName) {
<span class="cstat-no" title="statement not covered" >          throw new Error(`Unable to map page ${page.name} into tenant level page setting`);</span>
        }
        accum[pageName] = { ...page };
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!globalClient[0]) {
<span class="cstat-no" title="statement not covered" >      throw new Error('Unable to find global client id when trying to dump the login page');</span>
    }
&nbsp;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (globalClient[0].custom_login_page) {
      pages.push({
        name: 'login',
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!pages) <span class="cstat-no" title="statement not covered" >return;</span>
&nbsp;
    // Login page is handled via the global client
